...
<section class="preview-section"><h2>Live Display Preview</h2><canvas id="preview" class="preview" width="1280" height="64"></canvas><label for="previewSpeed">Preview speed: <span id="previewSpeedValue">70</span> ms</label><input id="previewSpeed" type="range" min="20" max="250" value="70" step="5"></section>
...
function drawPreview(now){const c=$('preview'),x=c.getContext('2d'),runs=previewRuns(),speed=+$('previewSpeed').value||70,level=.9+.1*(+$('brightness').value||25)/255;x.fillStyle='#020303';x.fillRect(0,0,1280,64);x.fillStyle='#0b1011';for(let py=0;py<8;py++)for(let px=0;px<160;px++){x.beginPath();x.arc(px*8+4,py*8+4,1,0,Math.PI*2);x.fill()}let cursor=previewX;x.globalAlpha=level;for(const run of runs){x.fillStyle=run[1];for(const ch of run[0].toUpperCase()){const glyph=LED_FONT[ch]||LED_FONT['?'];for(let col=0;col<5;col++){const screenX=cursor+col;if(screenX>=0&&screenX<160)for(let row=0;row<7;row++)if(glyph[col]&(1<<row)){x.beginPath();x.arc(screenX*8+4,row*8+4,3.2,0,Math.PI*2);x.fill()}}cursor+=6}}x.globalAlpha=1;if(now-lastPreview>=speed){previewX--;lastPreview=now}if(cursor<0)previewX=160;requestAnimationFrame(drawPreview)}
