...
*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font:15px/1.45 system-ui,sans-serif}header{border-bottom:1px solid var(--line);padding:20px}main{width:min(720px,calc(100% - 32px));margin:26px auto 50px}h1{font-size:22px;margin:0 0 4px}.status,.hint{color:var(--muted)}.dot{display:inline-block;width:9px;height:9px;border-radius:50%;background:var(--green);margin-right:7px}.dot.offline{background:var(--red)}section{padding:22px 0;border-bottom:1px solid var(--line)}h2{font-size:16px;margin:0 0 14px}label{display:block;color:var(--muted);margin:13px 0 6px}textarea,select,input[type=number]{width:100%;border:1px solid var(--line);background:var(--panel);color:var(--text);padding:11px;border-radius:6px;font:inherit}textarea{min-height:92px;resize:vertical}.row{display:grid;grid-template-columns:1fr 1fr;gap:16px}.range{display:flex;align-items:center;gap:12px}.range input{width:100%}button{border:0;border-radius:6px;padding:10px 15px;background:var(--orange);color:#17120d;font-weight:700;cursor:pointer}button.secondary{background:#30363b;color:var(--text)}button.danger{background:#e5484d;color:white}.actions{display:flex;gap:10px;margin-top:18px;flex-wrap:wrap}.hint{font-size:13px}.ok{color:var(--green);min-height:22px;margin-top:10px}.preview{width:100%;height:auto;aspect-ratio:20/1;background:#030303;border:1px solid #30363b;image-rendering:pixelated;display:block}.debug-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1px;background:var(--line);border:1px solid var(--line)}.debug-item{background:var(--panel);padding:12px}.debug-item span{display:block;color:var(--muted);font-size:12px}.debug-item strong{display:block;margin-top:3px;overflow-wrap:anywhere}.problem{margin-top:12px;padding:12px;border-left:4px solid var(--green);background:var(--panel)}.problem.error{border-color:var(--red);color:var(--red)}code{background:var(--panel);padding:2px 5px;border-radius:4px}@media(max-width:560px){.row,.debug-grid{grid-template-columns:1fr}}
...
<section><h2>Live Display Preview</h2><canvas id="preview" class="preview" width="160" height="8"></canvas></section>
...
const $=id=>document.getElementById(id);let latest={btc:0,eth:0,eurUsd:0,btcChange24h:0,ethChange24h:0};let previewX=160,lastPreview=0;$('brightness').oninput=()=>{$('brightnessValue').textContent=$('brightness').value};
...
function signed(v){return(v>=0?'+':'')+Number(v||0).toFixed(1)+'%'}
function previewText(){const mode=$('mode').value,msg=$('message').value.trim();if(mode==='message')return msg||'EXTRA MESSAGE EMPTY';if(mode==='clock')return new Date().toLocaleTimeString('en-US',{hour:'numeric',minute:'2-digit'});let prices='BTC $'+Math.round(latest.btc||0).toLocaleString('en-US')+' '+signed(latest.btcChange24h)+'   ETH $'+Math.round(latest.eth||0).toLocaleString('en-US')+' '+signed(latest.ethChange24h)+'   EUR/USD '+Number(latest.eurUsd||0).toFixed(4);return mode==='all'&&msg?prices+'   '+msg:prices}
function drawPreview(now){const c=$('preview'),x=c.getContext('2d'),text=previewText(),speed=Math.max(1,+$('speed').value||12);x.fillStyle='#030303';x.fillRect(0,0,160,8);x.globalAlpha=.25+(+$('brightness').value||25)/340;x.font='8px monospace';x.textBaseline='top';x.fillStyle=text.startsWith('BTC')?(latest.btcChange24h>=0?'#00ff38':'#ff3131'):'#ffffff';x.fillText(text,previewX,-1);x.globalAlpha=1;if(now-lastPreview>=speed){previewX--;lastPreview=now}if(previewX+x.measureText(text).width<0)previewX=160;requestAnimationFrame(drawPreview)}
...
void showPriceWithChart(const char* symbol, float price, float previous,
  ...
  String changeText = String(change24h >= 0 ? "+" : "") + String(change24h, 1) + "%";
