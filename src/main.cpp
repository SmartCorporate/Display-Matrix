...
const $=id=>document.getElementById(id);let latest={btc:0,eth:0,eurUsd:0,btcChange24h:0,ethChange24h:0};let previewX=160,lastPreview=0;const previewBuffer=document.createElement('canvas');previewBuffer.width=160;previewBuffer.height=8;$('brightness').oninput=()=>{$('brightnessValue').textContent=$('brightness').value};$('previewSpeed').oninput=()=>$('previewSpeedValue').textContent=$('previewSpeed').value;
...
function drawPreview(now){const c=$('preview'),x=c.getContext('2d'),b=previewBuffer.getContext('2d'),runs=previewRuns(),speed=+$('previewSpeed').value||70;b.clearRect(0,0,160,8);b.font='bold 8px monospace';b.textBaseline='top';let cursor=previewX;for(const run of runs){b.fillStyle=run[1];b.fillText(run[0],cursor,-1);cursor+=b.measureText(run[0]).width}x.fillStyle='#020303';x.fillRect(0,0,1280,64);const pixels=b.getImageData(0,0,160,8).data,level=.85+.15*(+$('brightness').value||25)/255;for(let py=0;py<8;py++)for(let px=0;px<160;px++){const i=(py*160+px)*4,lit=pixels[i+3]>72;x.beginPath();x.arc(px*8+4,py*8+4,lit?3.25:1,0,Math.PI*2);x.fillStyle=lit?'rgba('+pixels[i]+','+pixels[i+1]+','+pixels[i+2]+','+level+')':'#0b1011';x.fill()}if(now-lastPreview>=speed){previewX--;lastPreview=now}if(cursor<0)previewX=160;requestAnimationFrame(drawPreview)}
